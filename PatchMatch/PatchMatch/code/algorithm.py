    ...
    if odd_iteration == True:
        ...
        for i in range(h_min,-1,-1):
            for j in range(w_min,-1,-1):
                if propagation_enabled:
                    d_set = dict()
                    if(within_dim([i,j]+new_f[min(i+1,h_min-1),j],source_patches) == False):
                        loc_in_B = [i,j]+new_f[min(i+1,h_min-1),j]
                        new_score = np.linalg.norm(source_patches[i,j]-target_patches[loc_in_B[0],loc_in_B[1]])
                        d_set[(min(i+1,h_min-1),j)] = new_score
                    if(within_dim([i,j]+new_f[i,min(j+1,w_min-1)],source_patches) == False):
                        loc_in_B = [i,j]+new_f[i,min(j+1,w_min-1)]
                        new_score = np.linalg.norm(source_patches[i,j]-target_patches[loc_in_B[0],loc_in_B[1]])
                        d_set[(i,min(j+1,w_min-1))] = new_score
                    d_set[(i,j)] = best_D[i,j]
                    (x,y) = get_key_from_dict(d_set)
                    if((x,y)!=(i,j)):
                        new_f[i,j] = new_f[x,y]
